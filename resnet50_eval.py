...
def evaluate():
    ...
    model = keras.models.load_model(
        # 'C:\\DATA\\Python-data\\CIFAR-10\\models\\resnet_50.h5')
        'models/resnet_50.h5')
    # Load classes
    classes = {}
    ...
    categories = os.listdir('data/images/test/')
    # Get a category a random
    category = random.choice(categories)
    # Print the category
    print(category)
    # Get images in a category
    images = os.listdir('data/images/test/' + category)
    ...
    for i, name in enumerate(images):
        ...
        print(name)
        # Get the image
        image = cv2.imread('data/images/test/' + category + '/' + name)
        # Get input reshaped and rescaled
        input = np.array(image).astype('float32') / 255
        # Get predictions
        ...
        cv2.putText(
            image,
            '{0}: {1} %'.format(classes[prediction],
                                str(round(predictions[prediction] * 100, 2))),
        ...
        cv2.putText(image, '{0}'.format('CORRECT!' if correct else 'WRONG!'),
                    (12, 50), cv2.FONT_HERSHEY_DUPLEX, 0.7, (0, 0, 0), 2)
        cv2.putText(image, '{0}'.format('CORRECT!' if correct else 'WRONG!'),
                    (10, 48), cv2.FONT_HERSHEY_DUPLEX, 0.7,
    ...
    cv2.imwrite('data/images/predictions.jpg',
